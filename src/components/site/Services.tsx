...
const STEPS = [
  {
    ...
    note: "Фиксируем ваши критерии: марку, модель, комплектацию и бюджет.",
  },
  {
    num: "02",
    title: "Аванс 100 000 ₽",
    note: "Резервируем место в поставке и приступаем к подбору.",
  },
  {
    num: "03",
    title: "Выбираете автомобиль",
    note: "Из актуального прайса — цвет кузова, салона и комплектация.",
  },
  {
    num: "04",
    title: "Осмотр и бронирование",
    note: "Фото- и видеоотчёт с автомобилем. Оформляем бронь у экспортёра.",
  },
  {
    num: "05",
    title: "Оплата инвойса",
    note: "Переводите стоимость авто в юанях через ВТБ — займёт 5–10 минут.",
  },
  {
    num: "06",
    title: "Деньги поступают экспортёру",
    note: "Обычно 2–3 рабочих дня. Сразу готовим экспортные документы.",
  },
  {
    num: "07",
    title: "Оплата доставки",
    note: "Фиксированная стоимость логистики до границы с Россией.",
  },
  {
    num: "08",
    title: "Везём автомобиль в Россию",
    note: "Маршрут: Китай → нейтральная зона → Казахстан → СВХ в РФ.",
  },
  {
    num: "09",
    title: "Таможенное оформление",
    note: "Берём на себя все процедуры растаможки.",
  },
  {
    num: "10",
    title: "Оплата таможенной пошлины",
    note: "Выставляем квитанции — вы оплачиваете по фактическим ставкам.",
  },
  {
    num: "11",
    title: "Сертификация",
    note: "Лаборатория, СБКТС и электронный ПТС — авто готово к регистрации.",
  },
  {
    num: "12",
    title: "Доставка до вашего города",
    note: "Отправляем автовозом или транспортной компанией.",
  },
  {
    num: "13",
    title: "Мойка и подготовка",
    note: "Приводим автомобиль в порядок перед передачей.",
  },
  {
    num: "14",
    title: "Передаём вам ключи",
    note: "Осматриваем вместе и подписываем акт приёма.",
  },
  {
    num: "15",
    title: "ТО и допоборудование",
    note: "По желанию — сразу сделаем первое ТО и установим всё нужное.",
  },
];

/* Номера тёмных карточек (1-indexed) */
const DARK_CARDS = new Set(["03", "07", "09", "13"]);

const COLS = 3;

/* Водяной знак — логотип ТМ Авто */
function LogoWatermark({ dark }: { dark: boolean }) {
  return (
    <svg
      viewBox="0 0 187 89"
      aria-hidden="true"
      className="pointer-events-none absolute bottom-3 right-3 h-10 w-auto select-none"
      style={{ opacity: dark ? 0.15 : 0.07 }}
...
function HLine({ dark }: { dark: boolean }) {
  return (
    <div className="flex items-center" aria-hidden="true">
      <div
        className={`h-px w-full rounded-full ${dark ? "bg-white/20" : "bg-steel/30"}`}
      />
    </div>
  );
}
      ...
      <Container>
        ...
        <div className="mt-8 md:mt-12">
          {rows.map((row, rowIndex) => {
            /* Считаем есть ли тёмные карточки в строке, чтобы корректно
               рисовать линию-коннектор между ними */
            return (
              <div key={rowIndex}>
                <div
                ...
                >
                  {row.map((step, ci) => {
                    ...
                    return (
                      <div key={step.num} className="contents">
                        ...
                        >
                          <span className="mb-2 tabular-nums text-[0.875rem] font-bold leading-none text-brand">
                            {step.num}
                          </span>
                          <p
                            className={`text-[1rem] font-semibold leading-snug ${dark ? "text-white" : "text-ink"}`}
                          >
                            {step.title}
                          </p>
                          {step.note && (
                            <p
                              className={`mt-2 text-[0.8125rem] leading-relaxed ${dark ? "text-white/65" : "text-steel"}`}
                            >
                              {step.note}
                            </p>
                          )}
                  ...
                  {row.length < COLS &&
                    Array.from({ length: COLS - row.length }).flatMap(
                      (_, i) => [
                        <HLine key={`eh-${i}`} dark={false} />,
                        <div key={`ep-${i}`} />,
                      ],
                    )}
                </div>
