...
function HLine({ dark }: { dark: boolean }) {
  return (
    <div className="flex items-center" aria-hidden="true">
      <div className={`h-px w-full rounded-full ${dark ? 'bg-white/20' : 'bg-steel/30'}`} />
    </div>
  );
}
                    ...
                    return (
                      <>
                        ...
                        >
                          ...
                          <p className={`text-[1rem] font-semibold leading-snug ${dark ? 'text-white' : 'text-ink'}`}>
                            {step.title}
                          </p>
                          {step.note && (
                            <p className={`mt-2 text-[0.8125rem] leading-relaxed ${dark ? 'text-white/65' : 'text-steel'}`}>
                              {step.note}
                            </p>
                          )}
